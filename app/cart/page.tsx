...
export default function CartPage() {
  ...
  const totalRewardPoints = useMemo(() => Math.ceil(subtotal * 0.1), [subtotal]);

  return (
    <main className="min-h-screen w-full bg-[#f4ead5] pb-24 md:pb-8">
      <header className="border-b-2 border-[#f00] p-4 md:p-6">
        <p className="text-[11px] font-extrabold uppercase tracking-[0.15em] text-[#1034b8]">Your order</p>
          ...
          items.map((item, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="min-w-8 text-center text-base font-black text-[#1f2937]">{item.quantity}</p>
                <button
                  type="button"
                  onClick={() => setItemQuantity(item.id, item.quantity + 1)}
                  className="border border-[#1034b8] px-3 py-1 text-sm font-black text-[#1034b8]"
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 p-4 md:p-6">
        <div className="grid gap-3">
          <p className="text-xl font-black uppercase text-[#f00]">Subtotal: {formatUSD(subtotal)}</p>
